  ...
  try {
    ...
    for (const feature of polygons.features) {
      if (feature.geometry && feature.geometry.type === 'MultiPolygon') {
        ...
        const isInside = turf.booleanPointInPolygon(point, feature as turf.Feature<turf.MultiPolygon>);
        if (isInside) {
          return true;
        }
      } else if (feature.geometry && feature.geometry.type === 'Polygon') {
        // Single Polygon
        const isInside = turf.booleanPointInPolygon(point, feature as turf.Feature<turf.Polygon>);
        if (isInside) {
          return true;
        }
      }
