...
class MLEnhancedVisualizations {
  constructor(map, mlService) {
    ...
    this.isDeckGL = map && map.setProps; // Detect deck.gl vs Leaflet
    
    console.log('🎨 MLEnhancedVisualizations inicializado', this.isDeckGL ? '(deck.gl)' : '(Leaflet)');
  }
  ...
  async createSpeciesClusters() {
    try {
      ...
      if (this.layers.has('speciesClusters')) {
        this.removeLayer('species-clusters');
      }
  ...
  removeLayer(layerId) {
    if (this.isDeckGL) {
      ...
      // Leaflet layer removal
      const layer = this.layers.get(layerId);
      if (layer && this.map.removeLayer) {
        this.map.removeLayer(layer);
      }
    }
  }
  
  async generateHeatmapDataForDeckGL() {
    const points = [];
    ...
    angolaOceanicCoords.forEach(coord => {
      ...
      for (let i = 0; i < 8; i++) {
        ...
        points.push({
          latitude: coord[0] + latOffset,
          longitude: coord[1] + lngOffset,
  ...
  async generateClusterDataForDeckGL() {
    const species = ['Sardinha', 'Cavala', 'Atum', 'Anchoveta', 'Carapau'];
    const colors = [
      [255, 99, 132, 200],  // Red
      [54, 162, 235, 200],  // Blue  
      [255, 205, 86, 200],  // Yellow
      [75, 192, 192, 200],  // Teal
      [153, 102, 255, 200]  // Purple
    ];
    
    return species.map((specie, index) => ({
      longitude: 9 + Math.random() * 5,
      latitude: -16 + Math.random() * 8,
      species: specie,
      size: 2 + Math.random() * 8,
      population: Math.floor(1000 + Math.random() * 9000),
      confidence: Math.floor(70 + Math.random() * 30),
      color: colors[index] || [128, 128, 128, 200]
    }));
  }
