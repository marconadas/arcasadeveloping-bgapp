...
export function ImprovedBaseMapSelector({ onLayerChange, className = '' }: BaseMapSelectorProps) {
  ...
  const hoverClass = isDark ? 'hover:bg-slate-800/80' : 'hover:bg-gray-50/80';
  ...
  return (
    <div className={`absolute top-4 right-4 z-[1000] ${className}`}>
      <div className={`${panelClass} rounded-lg shadow-lg border min-w-[220px]`}>
        {/* Current Layer Display with Status */}
        <button
          onClick={() => setIsExpanded(!isExpanded)}
          className={`w-full px-4 py-3 flex items-center justify-between ${hoverClass} rounded-lg transition-colors`}
          title={LAYER_DESCRIPTIONS[currentLayer]}
        >
          <div className="flex items-center gap-2">
            <Map className={`w-4 h-4 ${secondaryTextClass}`} />
        ...
        {hasError && !isExpanded && (
          <button
            ...
            className={`w-full px-4 py-2 flex items-center justify-center gap-2 text-xs text-orange-600 dark:text-orange-400 ${isDark ? 'hover:bg-orange-900/30' : 'hover:bg-orange-50'} transition-colors border-t ${borderClass}`}
          >
            ...
            {Object.entries(MAP_LAYERS).map(([id, config]) => (
              <button
                ...
                className={`w-full px-4 py-2.5 text-left transition-colors flex items-center justify-between ${
                  id === currentLayer ? activeClass : `${isDark ? 'text-gray-300' : 'text-gray-700'} ${hoverClass}`
                }`}
              ...
              >
                ...
                {id === currentLayer && (
                  <span className={`text-xs ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>Active</span>
                )}
              </button>
            ))}
          </div>
      ...
      {/* Status Information */}
      {(isLoading || hasError) && (
        <div className={`mt-2 ${panelClass} rounded-lg shadow-lg border px-3 py-2`}>
          {isLoading && (
