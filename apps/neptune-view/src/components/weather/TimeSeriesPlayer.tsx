...
export default function TimeSeriesPlayer({
  ...
  const timestamp = currentFrameData ? formatTimestamp(currentFrameData.timestamp) : { date: '--', time: '--' };
        ...
        <div className="mt-4 pt-4 border-t border-gray-700">
          <div className="grid grid-cols-3 gap-2 text-xs text-gray-400">
            <div>
              ...
              <div className="font-medium">{currentFrameData.dataPoints.toLocaleString()}</div>
            </div>
            <div>
              <div className="text-gray-500">Mín/Máx</div>
              <div className="font-medium">
                {currentFrameData.minValue.toFixed(2)} / {currentFrameData.maxValue.toFixed(2)}
              </div>
            </div>
            <div>
              <div className="text-gray-500">Média</div>
              <div className="font-medium">{currentFrameData.avgValue.toFixed(2)}</div>
            </div>
          </div>
        </div>
      )}
