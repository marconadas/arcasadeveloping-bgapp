...
export default function MLDashboard({
  ...
  const advancedMetrics = useMemo(() => {
    ...
    const modelMetrics = models.map(model => {
      const modelPreds = recentPredictions.filter(p => (p as any).modelId === model.id || p.model_name === model.id || p.prediction_type === model.type);
      ...
      const typeDistribution = modelPreds.reduce((acc, p) => {
        const pType = p.prediction_type || (p as any).type;
      ...
      <div className={contentClasses}>
        ...
        <div className="flex items-center justify-between p-6 border-b dark:border-gray-700">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <RefreshCw className={`w-5 h-5 text-gray-600 dark:text-gray-400 ${isLoadingPredictions ? 'animate-spin' : ''
                }`} />
            </button>
            <button
            ...
            <div className="flex gap-2">
              {(['24h', '7d', '30d'] as const).map(period => (
                <button
                  ...
                  className={`px-3 py-1 text-sm rounded-lg transition-colors ${selectedMetricPeriod === period
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-4">
              ...
              >
                ...
                <ChevronRight className={`w-4 h-4 transition-transform ${showModelComparison ? 'rotate-90' : ''
                  }`} />
              </button>
            </div>
            ...
            <div className="space-y-3">
              {advancedMetrics.modelMetrics.map(metric => (
                <div
                  key={metric.model.id}
                  className={`p-4 rounded-lg border transition-all cursor-pointer ${metric.isActive
            ...
            <div className="mb-6">
              ...
              <div className="space-y-2">
                {alerts.slice(0, 5).map(alert => (
                  <div
                    key={alert.id}
                    className={`p-3 rounded-lg border flex items-start justify-between ${(alert.type === 'anomaly' || alert.type === 'high-risk')
                        ? 'border-red-300 bg-red-50 dark:bg-red-900/20'
                        : 'border-yellow-300 bg-yellow-50 dark:bg-yellow-900/20'
                      } ${alert.isRead ? 'opacity-60' : 'opacity-100 shadow-sm'}`}
                  >
                    <div className="flex items-start gap-3">
                      {(alert.type === 'anomaly' || alert.type === 'high-risk') ? (
