...
export class URLReplacerSiliconValley {
  private static readonly LOCALHOST_PATTERNS = [
    { pattern: /http:\/\/localhost:8085/g, replacement: () => ENV.scientificInterfacesUrl },
    { pattern: /http:\/\/localhost:8000/g, replacement: () => ENV.apiUrl },
    { pattern: /http:\/\/localhost:5080/g, replacement: () => ENV.externalServices.pygeoapi },
    ...
    { pattern: /http:\/\/localhost:8083/g, replacement: () => ENV.isDevelopment ? 'http://localhost:8083' : 'https://bgapp-auth.pages.dev' },
    { pattern: /localhost:8085/g, replacement: () => ENV.scientificInterfacesUrl.replace('https://', '').replace('http://', '') },
    { pattern: /localhost:8000/g, replacement: () => ENV.apiUrl.replace('https://', '').replace('http://', '') },
  ];
