...
export function RealTimeMap({
  ...
  // Generate unique key on component mount to prevent "Map container already initialized" errors
  const [mapKey] = useState(() => `map-${Date.now()}`);

  useEffect(() => {
    setIsClient(true);

    // Cleanup function to properly remove map container on unmount
    return () => {
      if (mapRef.current) {
        try {
          ...
          console.warn('[RealTimeMap] Cleanup error:', error);
        }
      }
    };
  }, []);

  // Note: Map auto-remounts with unique key, no manual reset needed
  ...
  // Fetch ML predictions data independently with filtering support
  const fetchMLPredictionsDirect = useCallback(async () => {
    try {
      console.log('[RealTimeMap] Fetching ML predictions with filters:', {
        models: activeMLModels,
        minConfidence: mlMinConfidence
      });
      ...
      // Build query parameters with ML filters
      const params = new URLSearchParams({
        ...
        minConfidence: mlMinConfidence.toString()
      });

      // Add prediction types if any are active
      if (activeMLModels.length > 0) {
        params.append('predictionType', activeMLModels.join(','));
      }

      const response = await fetch(
        `/api/realtime/ml-predictions?${params.toString()}`,
        {
          cache: 'no-store',
          headers: {
            'Accept': 'application/json'
      ...
      const data = await response.json();
      console.log('[RealTimeMap] ML API response (filtered):', {
        hasData: !!data.data,
  ...
  }, [activeMLModels, mlMinConfidence]);
  ...
  return (
    <div className={`relative w-full h-full rounded-lg overflow-hidden ${className}`}>
      ...
      >
        ...
        {/* ML Predictions Layer - Prioritize filtered props over local fetch */}
        {(() => {
          ...
          // FIXED: Use filtered props first (from page.tsx), fallback to local fetch
          const mlDataToUse = mlPredictions.length > 0 ? mlPredictions : localMLPredictions;
          console.log(`[RealTimeMap] Rendering MLPredictionsLayer with ${mlDataToUse.length} predictions (source: ${mlPredictions.length > 0 ? 'filtered props' : 'local fetch'})`);
      ...
      {/* Overlay de informações - movido para canto superior direito */}
      {chloroplethData.length > 0 && (
        <div className={`absolute top-4 right-4 z-[500] ${styles.mapOverlay} rounded-lg p-2 pointer-events-none`}>
          <div className={`text-xs ${styles.secondaryText} flex items-center gap-1.5 font-medium opacity-75`}>
            <span className="text-xs">📊</span>
            <span>{chloroplethData.length} dados</span>
          </div>
        </div>
      )}
